...
export function TemplatesGalleryClient({ customTemplates }: TemplatesGalleryClientProps) {
  ...
  const filtered = allTemplates.filter((tmpl) => {
    ...
    const matchesCat =
      category === 'all' ||
      (category === 'personalizados' && tmpl.category === 'personalizados') ||
      (category !== 'personalizados' && tmpl.documentType === category) ||
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((tmpl) => (
          <div
            key={tmpl.id}
            className="bg-card rounded-3xl p-6 border border-border main-container-shadow card-shadow-hover flex flex-col justify-between space-y-6 group"
          >
            <div>
              {/* Header pill */}
              <div className="flex justify-between items-center mb-4">
                ...
                {tmpl.isPreset ? (
                  ...
                  <span className="text-[11px] font-bold text-muted-foreground">Personalizado</span>
                )}
              </div>

              {/* Title & Description */}
              <h3 className="font-heading font-bold text-lg text-foreground group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                {tmpl.name}
              </h3>
            ...
            <div className="pt-4 border-t border-border flex items-center gap-2">
              <Link href={`/documents/new/${tmpl.documentType}`} className="flex-1">
                <Button className="w-full bg-primary text-primary-foreground hover:bg-primary/80 rounded-xl text-xs font-semibold h-10 shadow-sm gap-1.5">
