...
const features = [
  {
    icon: FileText,
    title: 'Orçamentos',
    desc: 'Crie orçamentos comerciais detalhados com tabela de itens, descontos e cálculo automático.',
    color: 'bg-slate-900',
  },
  {
    icon: TrendingUp,
    title: 'Propostas',
    desc: 'Elabore propostas executivas com escopo, cronograma de entregas e investimento.',
    color: 'bg-blue-600',
  },
  {
    icon: Wallet,
    title: 'Recibos',
    desc: 'Emita recibos e comprovantes de pagamento formais com valor por extenso.',
    color: 'bg-slate-900',
  },
  {
    icon: Clipboard,
    title: 'Ordens de Serviço',
    desc: 'Gere ordens técnicas com diagnóstico, peças, mão de obra e laudo.',
    color: 'bg-blue-600',
  },
  {
    icon: FileSignature,
    title: 'Contratos',
    desc: 'Redija contratos de serviços com cláusulas jurídicas, prazos e assinaturas.',
    color: 'bg-slate-900',
  },
  {
    icon: Users,
    title: 'Clientes',
    desc: 'Armazene e gerencie seus clientes com CPF/CNPJ, e-mail e contatos.',
    color: 'bg-blue-600',
  },
  {
    icon: Building2,
    title: 'Empresas',
    desc: 'Cadastre suas empresas emissoras e alterne entre elas com um clique.',
    color: 'bg-slate-900',
  },
  {
    icon: Download,
    title: 'Exportação em PDF',
    desc: 'Gere PDFs diagramados, prontos para impressão ou envio por e-mail.',
    color: 'bg-blue-600',
  },
]

const templateTypes = [
  { title: 'Orçamento Comercial', type: 'orcamento', desc: 'Tabela de itens, descontos e condições de pagamento.', tag: 'Mais Usado' },
  { title: 'Proposta Comercial', type: 'proposta', desc: 'Escopo, cronograma, investimento e termos de aceite.', tag: 'Editorial' },
  { title: 'Recibo de Pagamento', type: 'recibo', desc: 'Comprovante formal com valor por extenso.', tag: 'Essencial' },
  { title: 'Ordem de Serviço', type: 'ordem-servico', desc: 'Diagnóstico, peças, mão de obra e garantia.', tag: 'Técnico' },
  { title: 'Contrato de Serviços', type: 'contrato', desc: 'Cláusulas jurídicas, prazos e assinaturas.', tag: 'Jurídico' },
]

const steps = [
  { title: 'Cadastre sua empresa', desc: 'Informe os dados da sua empresa emissora para personalizar seus documentos.' },
  { title: 'Escolha um modelo', desc: 'Selecione entre orçamento, proposta, recibo, ordem de serviço ou contrato.' },
  { title: 'Preencha os dados', desc: 'Preencha cliente, itens e condições. O valor é calculado automaticamente.' },
  { title: 'Exporte em PDF', desc: 'Gere um PDF diagramado de alta resolução, pronto para enviar ou imprimir.' },
]
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#f8f9fc] text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* Recursos Section */}
      <section id="recursos" className="py-20 bg-white border-y border-slate-200/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
            <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">Recursos</span>
            <h2 className="font-heading text-3xl sm:text-4xl font-bold text-slate-900">
              Tudo que você precisa para emitir documentos profissionais
            </h2>
            <p className="text-slate-600">
              Do orçamento ao contrato, gerencie todo o ciclo comercial do seu negócio em um só lugar.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature) => {
              ...
              return (
                <div key={feature.title} className="p-6 rounded-3xl bg-slate-50/70 border border-slate-100 card-shadow-hover space-y-4">
                  <div className={`w-11 h-11 rounded-2xl ${feature.color} text-white flex items-center justify-center shadow-sm`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <h3 className="font-heading font-bold text-base text-slate-900">{feature.title}</h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-2">Cinco modelos profissionais para cada necessidade comercial.</p>
            </div>
            <Link href="/register">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            {templateTypes.map((tmpl) => (
              <div key={tmpl.type} className="bg-white rounded-2xl p-6 border border-slate-200/80 card-shadow-hover flex flex-col justify-between space-y-4">
                <div>
                  <div className="flex justify-between items-center mb-3">
                    <span className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-medium uppercase">
                      {tmpl.type}
                    </span>
                    <span className="text-xs font-semibold text-blue-600">{tmpl.tag}</span>
                  </div>
                  <h4 className="font-heading font-bold text-base text-slate-900">{tmpl.title}</h4>
      ...
      <section id="como-funciona" className="py-20 bg-white border-y border-slate-200/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
            <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">Como Funciona</span>
            <h2 className="font-heading text-3xl sm:text-4xl font-bold text-slate-900">
              Crie seu primeiro documento em 4 passos
            </h2>
            <p className="text-slate-600">
              Simples, rápido e sem complicação. Do cadastro ao PDF em poucos minutos.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((step, idx) => (
              <div key={step.title} className="relative p-6 rounded-3xl bg-slate-50/70 border border-slate-100 card-shadow-hover space-y-4">
                <div className="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center font-heading font-bold text-sm shadow-sm">
                  {idx + 1}
                </div>
                <h3 className="font-heading font-bold text-base text-slate-900">{step.title}</h3>
      ...
      <footer className="py-10 bg-slate-950 text-slate-500 text-xs border-t border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            ...
            <a href="#recursos" className="hover:text-slate-300">Recursos</a>
            <a href="#templates" className="hover:text-slate-300">Templates</a>
            <a href="#como-funciona" className="hover:text-slate-300">Como Funciona</a>
          </div>
        </div>
      </footer>
    </div>
